# -*- coding: utf-8 -*-
import re, base64, json

# Official Randstad horizontal logo path (symbol + "randstad" wordmark), fill #007cc5
PATH = "m123.53 111.863s-30.32-30.32-30.33-30.32c-1.803-1.853-4.246-2.864-6.9-2.864h-50.895v19.365h55.723a15.8 15.8 0 0 1 11.245 4.657 15.782 15.782 0 0 1 4.656 11.244v55.713h19.355v-50.896c0-2.644-1.011-5.097-2.854-6.9m53.99-33.183c-2.643 0-5.096 1.011-6.899 2.864l-30.32 30.32c-1.842 1.802-2.853 4.255-2.853 6.899v50.896h19.355v-55.713c0-8.771 7.13-15.9 15.9-15.9h55.714v-19.367zm184.012-2.213c-9.373 0-19.035 6.619-23.501 16.111l-.15.31h-1.262v-14.207h-14.69v90.98h15.761v-41.355c0-23.17 10.074-38.14 25.644-38.14 3.365 0 6.298.52 8.842 1.452v-13.989c-3.375-.681-5.858-1.162-10.644-1.162m485.588 8.782c-5.919-5.928-14.84-8.802-27.287-8.802-10.243 0-20.236 2.834-29.709 8.371l.811 13.177c6.95-5.577 16.862-9.031 26.195-9.031 7.98 0 13.678 1.822 17.433 5.577 3.975 3.965 5.827 10.043 5.827 19.115v.59l-.58-.04c-6.71-.36-11.446-.36-17.994-.35-11.185-.01-26.225 3.084-34.926 11.786-4.816 4.816-7.26 10.874-7.26 17.983 0 8.351 2.404 14.98 7.14 19.726 5.597 5.587 14.599 8.541 26.034 8.541 16.452 0 25.073-9.612 28.017-13.748l.16-.23h1.202v11.725h13.968v-55.693c0-13.237-2.944-22.62-9.032-28.697zm-6.73 40.463v8.681c0 7.22-2.313 13.378-6.669 17.813-4.626 4.697-11.354 7.17-19.475 7.18-4.947 0-10.083-2.073-13.418-5.397-2.773-2.784-4.255-6.328-4.255-10.264 0-16.581 22.69-18.374 32.452-18.374 1.813 0 3.655.09 5.427.18 1.773.09 3.595.18 5.388.18zm-402.848-40.463c-5.928-5.928-14.85-8.802-27.296-8.802-10.244 0-20.237 2.834-29.71 8.371l.812 13.177c6.949-5.577 16.862-9.031 26.194-9.031 7.98 0 13.688 1.822 17.433 5.577 3.975 3.965 5.828 10.043 5.828 19.115v.59l-.581-.04c-6.699-.36-11.435-.36-17.994-.35-11.174-.01-26.224 3.084-34.916 11.786-4.826 4.816-7.27 10.874-7.27 17.983 0 8.351 2.404 14.98 7.15 19.726 5.588 5.587 14.59 8.541 26.024 8.541 16.452 0 25.073-9.612 28.017-13.748l.17-.23h1.192v11.725h13.978v-55.693c0-13.237-2.964-22.62-9.031-28.697zm-6.74 40.463v8.681c0 7.22-2.312 13.378-6.668 17.813-4.616 4.697-11.355 7.17-19.476 7.18-4.946 0-10.083-2.073-13.407-5.397-2.784-2.784-4.256-6.328-4.256-10.264 0-16.581 22.69-18.374 32.443-18.374 1.812 0 3.655.09 5.437.18 1.762.09 3.585.18 5.377.18zm101.113-40.804c-5.617-5.617-13.718-8.461-24.081-8.461-13.007 0-24.953 6.579-29.71 16.382l-.15.32h-1.261v-14.449h-15.22v90.96h15.77v-42.046c0-11.805 2.304-21.168 6.85-27.816 4.916-7.19 12.236-10.834 21.758-10.834 14.43.89 20.057 10.273 20.057 33.483v47.212h15.77v-54.27c0-13.739-3.294-23.992-9.783-30.48m153.632 32.312c-8.701-3.976-16.922-7.72-16.922-16.452 0-7.18 6.578-11.805 16.762-11.805 3.715 0 13.017 2.082 17.393 4.405l1.311-13.427c-6.969-2.103-13.177-3.495-20.507-3.495-10.513 0-19.055 2.984-24.692 8.621-4.786 4.787-7.21 11.225-7.21 19.146 0 14.018 12.106 19.936 22.79 25.163 8.772 4.275 17.043 8.33 17.043 16.21 0 3.565-1.172 6.55-3.485 8.862-4.446 4.446-11.525 4.927-14.349 4.927-4.505 0-13.678-1.933-20.617-6.148l-.82 13.868c8.19 4.236 17.472 4.796 24.672 4.796 8.5 0 16.271-2.413 21.858-6.819 6.339-4.996 9.693-12.356 9.693-21.308 0-16.09-12.176-21.638-22.92-26.544m77.893 42.165c-3.906 0-7.49-1.452-10.104-4.055-3.084-3.084-4.706-7.58-4.706-12.997v-51.138h24.362v-12.515h-24.362v-26.506l-15.78 5.067v21.438h-20.758v12.516h20.757v54.582c0 8.29 2.223 14.71 6.609 19.085 4.656 4.666 11.755 7.03 21.087 7.03 5.327 0 10.574-1.433 13.889-2.374v-13.027c-3.245 1.893-6.93 2.894-10.995 2.894m175.741-123.993v55.763h-1.222l-.16-.26c-5.938-9.593-16.031-14.449-30-14.449-10.663 0-19.525 3.445-26.324 10.244-8.26 8.26-12.807 20.967-12.807 35.766 0 16.432 4.977 30.83 13.658 39.502 6.589 6.599 15.1 9.933 25.293 9.933 15.58 0 26.185-7.65 30.911-14.82l.16-.25h1.202v12.817h15.06v-134.246zm0 88.957c0 11.135-3.805 21.578-10.183 27.957-4.687 4.696-10.544 7.079-17.413 7.079-6.178 0-11.545-2.283-15.931-6.779-7.74-7.94-10.244-20.477-10.043-28.788-.25-11.725 3.064-21.979 9.081-28.137 4.366-4.456 9.994-6.718 16.712-6.718 6.94 0 12.877 2.413 17.634 7.169 6.448 6.448 10.143 16.732 10.143 28.217m-314.343-88.957v55.763h-1.221l-.16-.26c-5.948-9.593-16.042-14.449-30-14.449-10.674 0-19.535 3.445-26.324 10.244-8.261 8.26-12.817 20.967-12.817 35.766 0 16.432 4.986 30.83 13.658 39.502 6.588 6.599 15.1 9.933 25.303 9.933 15.57 0 26.174-7.65 30.9-14.82l.16-.25h1.212v12.817h15.05v-134.246zm0 88.957c0 11.135-3.815 21.578-10.183 27.957-4.696 4.696-10.554 7.079-17.413 7.079-6.188 0-11.545-2.283-15.93-6.779-7.741-7.94-10.254-20.477-10.044-28.788-.25-11.725 3.054-21.979 9.082-28.137 4.366-4.456 9.983-6.718 16.712-6.718 6.939 0 12.867 2.413 17.623 7.169 6.448 6.448 10.153 16.732 10.153 28.217"

num = re.compile(r'[-+]?(?:\d*\.\d+|\d+\.?)(?:[eE][-+]?\d+)?')

def tokenize(d):
    toks = re.findall(r'[MmLlHhVvCcSsQqTtAaZz]|[-+]?(?:\d*\.\d+|\d+\.?)(?:[eE][-+]?\d+)?', d)
    return toks

def parse_subpaths(d):
    toks = tokenize(d)
    i = 0
    cx = cy = 0.0
    sx = sy = 0.0
    subs = []          # list of dict {pts:[(x,y)...]}
    cur = None
    cmd = None
    def newsub():
        return {"pts": []}
    nargs = {'M':2,'L':2,'H':1,'V':1,'C':6,'S':4,'Q':4,'T':2,'A':7,'Z':0}
    while i < len(toks):
        t = toks[i]
        if re.match(r'[A-Za-z]', t):
            cmd = t; i += 1
        rel = cmd.islower()
        C = cmd.upper()
        if C == 'Z':
            cx, cy = sx, sy
            continue
        n = nargs[C]
        args = [float(toks[i+k]) for k in range(n)]
        i += n
        if C == 'M':
            x,y = args
            if rel: x+=cx; y+=cy
            cx,cy = x,y; sx,sy=x,y
            cur = newsub(); subs.append(cur); cur["pts"].append((x,y))
            cmd = 'l' if rel else 'L'  # subsequent implicit lineto
        elif C == 'L':
            x,y = args
            if rel: x+=cx; y+=cy
            cx,cy=x,y; cur["pts"].append((x,y))
        elif C == 'H':
            x = args[0]
            if rel: x+=cx
            cx=x; cur["pts"].append((x,cy))
        elif C == 'V':
            y = args[0]
            if rel: y+=cy
            cy=y; cur["pts"].append((cx,y))
        elif C == 'C':
            x1,y1,x2,y2,x,y = args
            if rel: x1+=cx;y1+=cy;x2+=cx;y2+=cy;x+=cx;y+=cy
            cur["pts"] += [(x1,y1),(x2,y2),(x,y)]; cx,cy=x,y
        elif C == 'S':
            x2,y2,x,y = args
            if rel: x2+=cx;y2+=cy;x+=cx;y+=cy
            cur["pts"] += [(x2,y2),(x,y)]; cx,cy=x,y
        elif C == 'Q':
            x1,y1,x,y = args
            if rel: x1+=cx;y1+=cy;x+=cx;y+=cy
            cur["pts"] += [(x1,y1),(x,y)]; cx,cy=x,y
        elif C == 'T':
            x,y = args
            if rel: x+=cx;y+=cy
            cur["pts"].append((x,y)); cx,cy=x,y
        elif C == 'A':
            rx,ry,rot,laf,sf,x,y = args
            if rel: x+=cx;y+=cy
            cur["pts"].append((x,y)); cx,cy=x,y
    return subs

subs = parse_subpaths(PATH)
def bbox(pts):
    xs=[p[0] for p in pts]; ys=[p[1] for p in pts]
    return min(xs),min(ys),max(xs),max(ys)

for k,s in enumerate(subs):
    b=bbox(s["pts"])
    s["bb"]=b
    print(k, "x[%.1f,%.1f] y[%.1f,%.1f] cx=%.1f"%(b[0],b[2],b[1],b[3],(b[0]+b[2])/2))

# classify
sym=[s for s in subs if (s["bb"][0]+s["bb"][2])/2 < 270]
wrd=[s for s in subs if (s["bb"][0]+s["bb"][2])/2 >= 270]
def union(ss):
    xs0=min(s["bb"][0] for s in ss); ys0=min(s["bb"][1] for s in ss)
    xs1=max(s["bb"][2] for s in ss); ys1=max(s["bb"][3] for s in ss)
    return xs0,ys0,xs1,ys1
sb=union(sym); wb=union(wrd)
print("SYMBOL bbox:", tuple(round(v,2) for v in sb), "n=",len(sym))
print("WORD bbox:", tuple(round(v,2) for v in wb), "n=",len(wrd))

import json as _j
open("_logo_bbox.json","w").write(_j.dumps({"sym":sb,"wrd":wb,"path":PATH}))

BLUE="#007cc5"
# pad helper
def vb(b,p=2.0):
    return "%.2f %.2f %.2f %.2f"%(b[0]-p,b[1]-p,(b[2]-b[0])+2*p,(b[3]-b[1])+2*p)
sym_vb=vb(sb); wrd_vb=vb(wb)

# --- mark only (symbol), root viewBox crops out the wordmark ---
MARK='<svg xmlns="http://www.w3.org/2000/svg" viewBox="%s"><path d="%s" fill="%s"/></svg>'%(sym_vb,PATH,BLUE)

# --- full stacked lockup: symbol on top, wordmark below (like the supplied image) ---
# layout in a 320 x 189 canvas
sym_w,sym_h=175.0,82.5
wrd_w,wrd_h=300.0,64.66
OW=320.0
sym_x=(OW-sym_w)/2; sym_y=10.0
wrd_x=(OW-wrd_w)/2; wrd_y=sym_y+sym_h+22.0
OH=wrd_y+wrd_h+10.0
FULL=('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 %.0f %.2f">'
      '<svg x="%.2f" y="%.2f" width="%.2f" height="%.2f" viewBox="%s" preserveAspectRatio="xMidYMid meet"><path d="%s" fill="%s"/></svg>'
      '<svg x="%.2f" y="%.2f" width="%.2f" height="%.2f" viewBox="%s" preserveAspectRatio="xMidYMid meet"><path d="%s" fill="%s"/></svg>'
      '</svg>')%(OW,OH, sym_x,sym_y,sym_w,sym_h,sym_vb,PATH,BLUE, wrd_x,wrd_y,wrd_w,wrd_h,wrd_vb,PATH,BLUE)

def b64(s): return "data:image/svg+xml;base64,"+base64.b64encode(s.encode("utf-8")).decode()
with open("_logo_data.js","w",encoding="utf-8") as f:
    f.write("var LOGO=%s;\n"%json.dumps(b64(FULL)))
    f.write("var LOGO_MARK=%s;\n"%json.dumps(b64(MARK)))
print("wrote _logo_data.js (FULL stacked + MARK), canvas %.0fx%.2f"%(OW,OH))
# also save raw svgs for inspection
open("_logo_full.svg","w",encoding="utf-8").write(FULL)
open("_logo_mark.svg","w",encoding="utf-8").write(MARK)
